...
각 항목은 20개의 영상들의 예시와 데이터에 기반한 구체적인 수치를 포함해서 내용을 쉽게 풀어서 설명해주세요."""
    ...
    def run(self):
        ...
        if not self.keyword:  # 키워드가 입력되지 않았을 때 소개 페이지 표시
            ...
            st.markdown("""
            여러분은 콘텐츠 기획을 어떻게 하시나요? 저는 유튜브API를 통해 얻은 데이터를 활용하는데요.
            추출한 데이터를 Claude ai를 통해 시청자 참여도가 높은 콘텐츠의 제목 패턴과 댓글 데이터를 분석합니다.
            유튜브 인싸이트 뽑아먹기는 이러한 분석 과정을 자동화한 도구입니다. 실무에 활용할 수 있는 실용적인 콘텐츠 인사이트를 제공합니다.
