...
각 항목은 20개의 영상들의 예시와 데이터에 기반한 구체적인 수치를 포함해서 내용을 쉽게 풀어서 설명해주세요."""
    ...
    def run(self):
        ...
        if not self.keyword:  # 키워드가 입력되지 않았을 때 소개 페이지 표시
            st.title("🎥 YouTube 콘텐츠 분석 도구")
            
            # 섹션 1: 소개
            st.header("👋 소개")
            st.markdown("""
            안녕하세요! YouTube 콘텐츠 분석 도구에 오신 것을 환영합니다.
            
            여러분은 콘텐츠를 기획할 때 어디서 아이디어를 얻으시나요? 저는 유튜브에서 인사이트를 얻는 편인데요.
            시청자 참여도가 높은 콘텐츠들을 분석하고, 시청자들이 남긴 댓글에서 콘텐츠에 담을 키워드를 추출하는 방식으로 작업해왔습니다.
            유튜브 인사이트는 이러한 분석 과정을 자동화한 도구입니다. Claude AI를 활용하여 인기 콘텐츠의 제목과 댓글을 분석하고, 의미 있는 인사이트를 추출해 드립니다.
