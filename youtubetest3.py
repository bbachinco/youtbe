...
각 항목은 20개의 영상들의 예시와 데이터에 기반한 구체적인 수치를 포함해서 내용을 쉽게 풀어서 설명해주세요."""
    ...
    def run(self):
        ...
        if not self.keyword:  # 키워드가 입력되지 않았을 때 소개 페이지 표시
            ...
            st.markdown("""
            저는 콘텐츠를 기획할 때 유튜브에서 인사이트를 얻는 편인데요.
            시청자 참여도가 높은 콘텐츠들을 분석하고, 시청자들이 남긴 댓글에서 콘텐츠에 담을 키워드를 추출하는 방식으로 작업해왔습니다.
            유싸뽀는 이러한 분석 과정을 자동화한 도구입니다. Claude AI를 활용하여 인기 콘텐츠의 제목과 댓글을 분석하고, 의미 있는 인사이트를 추출해 드립니다.
            ...
            * 더 정확한 분석을 위해 충분한 수의 영상을 선택해주세요
            이 앱은 제 업무에 사용하기 위해 만들었고, 
            저처럼 작은 회사에서 일하는 마케터들을 위해 무료로 공유하고 있습니다. 
            부족한 점이 있더라도 이해해주시고, 업무에 조금이나마 도움이 되면 좋겠습니다.   
            더 궁금한 내용 있으시면 메일로 문의주세요.bbachinco@gmail.com
            """)
